...
//
int createDir(string prePath, string dirName, string &xDirPath, string &yDirPath)
{
    //
    ...
    xDirPath = prePath + "/" + splits[0]; // layer.yxfolders[dirName].xDirPath

    DIR *outdir = opendir(xDirPath.c_str());
    if(outdir == NULL)
    {
        //
        if(makeDir(xDirPath.c_str()))
        {
            cout<<"fail in makeDir "<<xDirPath<<endl;
            return -1;
        }
    }
    else
    ...
    yDirPath = xDirPath + "/" + splits[1]; // layer.yxfolders[dirName].yDirPath

    outdir = opendir(yDirPath.c_str());
    if(outdir == NULL)
    {
        //
        if(makeDir(yDirPath.c_str()))
        {
            cout<<"fail in makeDir "<<yDirPath<<endl;
            return -1;
        }
    }
    else
...
{
    p = NULL;
    wx = wy = wz = 1; // image size
    rx = ry = rz = 1; // region size
    xs = ys = zs = 1; // starting position
    lenkey = 1;
    datatype = 1;
    b_swap = 0;
    lenheader = 1;
...
char V3DRAWIO::checkMachineEndian()
{
...
//
void V3DRAWIO::swap2bytes(void *targetp)
{
    unsigned char * tp = (unsigned char *)targetp;
    unsigned char a = *tp;
    *tp = *(tp+1);
...
//
void V3DRAWIO::swap4bytes(void *targetp)
{
    unsigned char * tp = (unsigned char *)targetp;
    unsigned char a = *tp;
    *tp = *(tp+3);
...
int V3DRAWIO::canRead(char *filename)
{
    //
...
//    long long fileSize = _ftelli64(fid);
//#endif
...
//    lenheader = 4*4+2+1+lenkey;

//    //
//    long i;
//    if (strcmp(formatkey, keyread))
//    {
...
//    int tmpn=(int)fread(mysz, 4, 4, fid); //
//    if (tmpn!=4) {
//        if (keyread) {delete []keyread; keyread=0;}
//        return -1;
//    }
...
int V3DRAWIO::readlocal(FILE *fid, uint8* &p, long offsets, long xlocal, long ylocal, long zlocal, long xslocal, long yslocal, long zslocal)
{
    //
    if(!fid)
    {
...
//
int writeRAW(string filename, uint8* img, int viHeight, int viWidth, int viDepth, int type, int height, int width, int depth, int startx, int starty, int startz, int szMemory, int threads)
{
