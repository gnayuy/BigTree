...
public:
    ...
    //
    int read(char* filename, uint8* &img, int &viHeight, int &viWidth, int &viDepth, int &type, int &height, int &width, int &depth, int &startx, int &starty, int &startz, int szMemory, int threads);

    //
    int readlocal(FILE *fid, uint8* &p, long offsets, long xlocal, long ylocal, long zlocal, long xslocal, long yslocal, long zslocal);

    //
    int canWrite(char *filename);

    //
    int write(uint8* img, int viHeight, int viWidth, int viDepth, int type, int height, int width, int depth, int startx, int starty, int startz, int szMemory, int threads);


public:
    uint8 *p; // image pointer
    long wx,wy,wz; // image size
    long rx,ry,rz; // region size
    long xs, ys, zs; // starting position
    int datatype, b_swap;
    long lenkey, lenheader;
};
